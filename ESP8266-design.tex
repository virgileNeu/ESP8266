...
\begin{figure}[H]
        \makebox[\textwidth][c]{\includegraphics[width=0.9\textwidth,height=0.9\textheight,keepaspectratio]{EPFL-Logo-CMJN.eps}}
...
\begin{itemize}
    \item A control register CTRL,
    \item A status register STATUS,
    \item A register for the UART waiting cycles (depends on the UART rate),
    \item The FIFO\_out\_data register,
    \item The FIFO\_out\_free\_space register,
    \item The FIFO\_in\_data register,
    \item The FIFO\_in\_pending\_data register.
    \item The reset\_FIFO\_in register.
\end{itemize}
...
\begin{center}
\captionof{table}{Power consumption of the ESP8266 device}
...
\begin{center}
\captionof{table}{Throughput between the ESP8266 and another WiFi device}
...
\includepdf[pages={1}, scale=.82, pagecommand=\section{AT Command Set}]{ESP8266ATCommandsSet.pdf}
